...
XYZ_BOX = ArticulationCfg(
    spawn=sim_utils.UsdFileCfg(
        usd_path=f"{THOMAS_ASSETS}/MOVING/test_h1_xyz.usd",
    ...
    actuators={
        ...
        "z_1_actuator": ImplicitActuatorCfg(
            joint_names_expr=["rot_1_joint"],
            effort_limit=2000.0,
            velocity_limit=10.0,
            stiffness=1e5,
            damping=0.0,
        ),
        "z_2_actuator": ImplicitActuatorCfg(
            joint_names_expr=["rot_2_joint"],
...
def run_sim(sim: sim_utils.SimulationContext, scene: InteractiveScene) -> None:
    ...
    while simulation_app.is_running():
        ...
        if count % 100 == 0:
            ...
            all_joint_pos_target = torch.zeros_like(xyz_box.data.joint_pos_target)
            print("Find:")
            print(f"x_drive index: {xyz_box.find_joints('x_drive_joint')}")
            print(f"y_drive index: {xyz_box.find_joints('y_drive_joint')}")
            print(f"z_1_drive index: {xyz_box.find_joints('rot_1_joint')}")
            print(f"z_2_drive index: {xyz_box.find_joints('rot_2_joint')}\n")
           ...
           # all_joint_vel_target[0, :] = env_1_joint_vel_target
            ...
            all_joint_vel_target[:, 1] = -2.0
            xyz_box.set_joint_velocity_target(
                all_joint_vel_target[0,:],
                env_ids=[0],
            )

            # Set rotational position targets to 0 so only move in x y direction
            no_inner_rot = torch.zeros((args_cli.num_envs,1), device=args_cli.device)
